...
TELEGRAM_GROUP_ID = os.getenv('TELEGRAM_GROUP_ID')  # Group to send logs
...
# Log and notify functions
def log_and_notify(message, user_id, user_name):
    log_message = f"User @{user_name} ({user_id}): {message}"
    print(log_message)  # Log to console or file as needed
    bot.send_message(TELEGRAM_GROUP_ID, log_message)  # Send the log to a Telegram group

def log_error(error_message, user_id, user_name):
    log_message = f"Error for user @{user_name} ({user_id}): {error_message}"
    print(log_message)  # Log to console or file as needed
    bot.send_message(TELEGRAM_GROUP_ID, log_message)  # Send the error log to a Telegram group

# Function to send "I am alive" message every hour
def send_iam_alive():
    while True:
        bot.send_message(TELEGRAM_GROUP_ID, "I am alive!")
        time.sleep(3600)

def start_alive_thread():
    alive_thread = threading.Thread(target=send_iam_alive)
...
def list_files(message, folder_id=None, page=0, edit=False):
    ...
    if files:
        ...
        # Remove this block to avoid adding the second "Get All Links" button
        ...
        for file in files:
            if not file.get('folder', None):  # Skip folders, only get files
                ...
                if link:
                    ...
                    log_and_notify(f"Link generated for file: {file['name']}", user_id, username)
        
        if links:
            bot.send_message(user_id, "\n".join(links))
        else:
            bot.send_message(user_id, "No links found or failed to generate links.")
            log_error(f"Failed to generate links for folder {folder_id}", user_id, username)
    else:
...
def generate_navigation_buttons(folder_id, page, access_token):
    ...
    # Add buttons for each file or folder (paginate 10 files per page)
    ...
    # Add the "Get All Links" button at the end
    all_files_button = InlineKeyboardButton("📄 Get All Links", callback_data=f"getalllinks_{folder_id}")
    markup.add(all_files_button)
    ...
    # Add a "Home" button to navigate back to the root folder
    ...
    if response.status_code == 200:
        ...
        if file_name:
            ...
            if link:
                ...
                log_and_notify(f"Generated link for {file_name}", user_id, username)
            else:
                bot.send_message(user_id, "Failed to generate the link.")
                log_error(f"Failed to generate link for {file_name}", user_id, username)
        else:
            bot.send_message(user_id, "File name not found.")
            log_error("File name not found", user_id, username)
    else:
        bot.send_message(user_id, "File not found.")
        log_error("File not found", user_id, username)
...
if __name__ == '__main__':
    ...
    start_alive_thread()  # Start the "I am alive" thread
